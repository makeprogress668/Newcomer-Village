...
class Hotel(BaseModel):
    ...
    price_source: str = Field(default="estimated", description="价格来源: amap/estimated/reference")
    booking_url: Optional[str] = Field(default=None, description="预订平台跳转链接")
...
class POIInfo(BaseModel):
    """POI信息（精简版 amap-mcp-server 的 maps_text_search 只返回 id/name/address/typecode,
    其余字段需要通过 maps_search_detail 二次补全;因此 location 等字段可选。）"""
    id: str = Field(..., description="POI ID")
    name: str = Field(..., description="名称")
    type: str = Field(default="", description="类型")
    address: str = Field(default="", description="地址")
    location: Optional[Location] = Field(default=None, description="经纬度坐标(精简返回时缺失)")
    tel: Optional[str] = Field(default=None, description="电话")
    rating: Optional[float] = Field(default=None, description="评分(0-5)")
    cost: Optional[str] = Field(default=None, description="人均消费")
    photos: List[str] = Field(default_factory=list, description="图片URL列表")
    biz_type: Optional[str] = Field(default=None, description="行业类型,如风景名胜/博物馆")
    typecode: Optional[str] = Field(default=None, description="高德分类码,可用'|'分隔")
    level: Optional[str] = Field(default=None, description="景区评级,如AAAAA/AAAA/AAA")
