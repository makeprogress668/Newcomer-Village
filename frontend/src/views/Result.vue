<template>
  <div class="result-container">
            ...
            >
              ...
              <a-card v-if="day.hotel" size="small" class="hotel-card">
                <template #title>
                  ...
                  <a-tag
                    v-if="day.hotel.price_source && day.hotel.price_source !== 'amap'"
                    color="orange"
                    size="small"
                ...
                <div v-if="day.hotel.booking_url" style="margin-top: 12px">
                  <a
                ...
                >
                  <div class="meal-entry">
                    <div class="meal-main">
                      ...
                      <a-tag v-if="meal.estimated_cost" color="green">人均 ¥{{ meal.estimated_cost }}</a-tag>
                    </div>
                    <div v-if="meal.address" class="meal-address">📍 {{ meal.address }}</div>
                    <div v-if="meal.description" class="meal-description">{{ meal.description }}</div>
                  </div>
                </a-descriptions-item>
              </a-descriptions>
            </a-collapse-panel>
    ...
    <!-- 全局免责声明 -->
    <div v-if="tripPlan" class="disclaimer-footer">
      ⚠️ 酒店价格信息为预估区间，最终以预订平台（如携程）实际价格为准；景点门票请以官方为准。
    </div>
...
// 加载所有景点图片：仅对 image_url 为空的景点请求兜底
const loadAttractionPhotos = async () => {
  ...
  tripPlan.value.days.forEach(day => {
    day.attractions.forEach(attraction => {
      // 后端已注入的 image_url 优先；前端缓存 attractionPhotos 优先
      if (attraction.image_url || attractionPhotos.value[attraction.name]) {
        return
      }
      const promise = fetch(
        `http://localhost:8000/api/poi/photo?name=${encodeURIComponent(attraction.name)}&city=${encodeURIComponent(city)}`
      )
        .then(res => res.json())
        .then(data => {
...
// 获取景点图片：优先用后端注入的 image_url；否则查前端缓存；否则返回 SVG 占位
const getAttractionImage = (attraction: { name: string; image_url?: string }, index: number): string => {
  if (attraction.image_url) {
    return attraction.image_url
  }
  if (attractionPhotos.value[attraction.name]) {
    return attractionPhotos.value[attraction.name]
  }
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="400" height="300">
    ...
    <text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" font-family="sans-serif" font-size="24" font-weight="bold" fill="white">${attraction.name}</text>
  </svg>`
